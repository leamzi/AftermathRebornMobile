package com.tucomoyo.aftermath.Clases 
{
	...
	public class PageFlip extends Sprite 
	{
        ...
        private var flipImage:ImageFlip;        //the moving image that represents the flipping page
        private var cacheImage:ImageFlip;       //the non moving image that represents the other page
        private var step:Number;                //how quick our page will be flipping


		/**@private*/
		public function PageFlip(pages:Vector.<DisplayObject>,bookWidth:Number,bookHeight:Number,startOnPage:int = 0)
		{
			super();
            this.pages = pages;
		...
		private function initPage():void
		{
            ...
            {
                cacheImage = new ImageFlip(rasterizeDispObj(pages[currentPage]));
                flipImage = new ImageFlip(rasterizeDispObj(pages[currentPage]));
                addChild(cacheImage);
                addChild(flipImage);
            }
            else
